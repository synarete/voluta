...
#ifdef VOLUTA_FUSE_STATX
#undef FUSE_KERNEL_MINOR_VERSION
#define FUSE_KERNEL_MINOR_VERSION 34
#endif

#ifdef VOLUTA_FUSE_STATX

#define FUSE_STATX 50
...
static void statx_ts_to_fuse(const struct statx_timestamp *ts,
                               ...
                               struct fuse_statx *attr)
{
	...
	statx_ts_to_fuse(&stx->stx_atime, &attr->atime);
	statx_ts_to_fuse(&stx->stx_btime, &attr->btime);
	statx_ts_to_fuse(&stx->stx_ctime, &attr->ctime);
	statx_ts_to_fuse(&stx->stx_mtime, &attr->mtime);
	attr->rdev_major = stx->stx_rdev_major;
	attr->rdev_minor = stx->stx_rdev_minor;
	attr->dev_major = stx->stx_dev_major;
	attr->dev_minor = stx->stx_dev_minor;
	attr->mnt_id = stx->stx_mnt_id;
}
