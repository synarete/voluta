...
static struct voluta_ms_env *g_mountd_ms_env_inst;
...
static void mountd_init_process(void)
{
	...
	voluta_set_logmaskp(&g_mountd_log_mask);
}
...
static void mountd_require_cap_sys_admin(void)
{
	...
	if (cap == NULL) {
		voluta_die(errno, "failed to get cap");
	}
	err = cap_get_flag(cap, value, CAP_EFFECTIVE, &flag);
	if (err) {
		voluta_die(errno, "failed to get capability: value=%d", value);
	}
	cap_free(cap);

	if (flag != CAP_SET) {
		voluta_die(0, "does not have CAP_SYS_ADMIN capability");
	}
}
...
static void mountd_create_mse_inst(void)
{
	...
	err = voluta_mse_new(&g_mountd_ms_env_inst);
	if (err) {
...
static void mountd_trace_start(void)
{
	voluta_log_meta_banner(program_invocation_short_name, 1);
}

static void mountd_trace_finish(void)
{
	voluta_log_meta_banner(program_invocation_short_name, 0);
}

static void mountd_load_mntrules(void)
{
	g_mountd_mntrules = voluta_parse_mntrules(g_mountd_confpath);
}
...
static void mountd_destroy_mse_inst(void)
{
	if (g_mountd_ms_env_inst) {
		voluta_mse_del(g_mountd_ms_env_inst);
...
{
	int err;
	struct voluta_ms_env *ms_env = g_mountd_ms_env_inst;
...
static void mountd_getopt(void)
{
	...
	while (opt_chr > 0) {
		...
		if (opt_chr == 'f') {
			...
			voluta_log_mask_by_str(&g_mountd_log_mask, optarg);
		} else if (opt_chr == 'v') {
			mountd_show_version();
		} else if (opt_chr == 'h') {
			mountd_show_usage();
		} else if (opt_chr > 0) {
			voluta_die(0, "unsupported option: %s", optarg);
		}
	}
	if (optind < argc) {
	...
	if (!g_mountd_confpath) {
		voluta_die(0, "missing argument: %s", "conf");
	}
}
