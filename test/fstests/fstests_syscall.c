...
void vt_statvfs_err(const char *path, int err)
{
	...
	expect_err(voluta_sys_statvfs(path, &stv), err);
}
...
{
	expect_ok(voluta_sys_utimes(filename, tm));
}
...
{
	expect_ok(voluta_sys_mkdirat(dirfd, pathname, mode));
}
...
{
	expect_err(voluta_sys_fremovexattr(fd, name), err);
}
...
void vt_flistxattr_err(int fd, char *list, size_t size, int err)
{
	...
	expect_err(voluta_sys_flistxattr(fd, list, size, &len), err);
}
...
{
	expect_ok(voluta_sys_fiemap(fd, fm));
}
