...
static void ut_file_fallocate_read(struct ut_env *ute)
{
	...
	ut_file_fallocate_read_(ute, UT_MEGA - 1, VOLUTA_AG_SIZE + 2);
	ut_file_fallocate_read_(ute, UT_GIGA, UT_UMEGA);
	ut_file_fallocate_read_(ute, UT_TERA - 2, VOLUTA_AG_SIZE + 3);
}
...
static void ut_file_fallocate_truncate(struct ut_env *ute)
{
	...
	ut_file_fallocate_truncate_(ute, 0, VOLUTA_AG_SIZE);
	ut_file_fallocate_truncate_(ute, UT_MEGA - 1, VOLUTA_AG_SIZE + 2);
	ut_file_fallocate_truncate_(ute, UT_GIGA, UT_UMEGA);
	ut_file_fallocate_truncate_(ute, UT_TERA - 2, VOLUTA_AG_SIZE + 3);
}
...
{
	const voluta_lba_t lba_beg = off_to_nbk(off);
	const voluta_lba_t lba_end = off_to_nbk_up(off + len);
