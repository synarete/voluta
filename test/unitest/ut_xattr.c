...
static void ut_xattr_simple(struct ut_env *ute)
{
	...
	ut_xattr_simple_(ute, VOLUTA_NAME_MAX, 1);
	ut_xattr_simple_(ute, 1, VOLUTA_XATTR_VALUE_MAX);
	ut_xattr_simple_(ute, VOLUTA_NAME_MAX, VOLUTA_XATTR_VALUE_MAX);
}

static void ut_xattr_any_value(struct ut_env *ute)
{
	for (size_t i = 1; i <= VOLUTA_XATTR_VALUE_MAX; ++i) {
		ut_xattr_simple_(ute, VOLUTA_NAME_MAX, i);
	}
}
